...
class HDMClouds():
    def __init__(self, data, alpha=0., lamb=1., n_center=200, back_level=None, ig_method="determinant",
        ...
        # gaussian center points and evaluation points
        xc = mu_red[:,0]; xe = mu_red[:,0]
        yc = mu_red[:,1]; ye = mu_red[:,1]

        # covariance matrix truncation
        if ig_method=="determinant":
            sig_red = np.asarray( [np.linalg.det(cov)**(1./4) for cov in sig_red] )
        elif ig_method=="eigenvalue":
            sig_red = np.asarray( [np.mean((np.linalg.eig(cov)[0]))**(1./2) for cov in sig_red] )
        ...
        # fast evaluation
        ########################################
        neigh_indexes,neigh_indexes_aux = compute_neighbors(mu_red, mu_red, 5*np.max(sig_red))
    ...
    def normalized_w(self):
        ...
        """
        w = self.get_params_mapped()
        d = len(self.dims)
        w = w * (2*np.pi*self.sig**2)**(d/2.)
        return w


    def get_approximation(self):
        w = self.get_params_mapped()
        u = gm_eval_fast(w, self.sig, self.xc, self.yc, self.xgrid, self.ygrid, self.nind3, self.nind_aux3)
    ...
    def F(self, params):
        ...
        u = gm_eval_fast(w, self.sig, self.xc, self.yc, self.xe, self.ye, self.nind1, self.nind_aux1)
        ...
        fb = self.fb
        #u_boundary = gm_eval_fast(w, self.sig, self.xc, self.yc, self.xb, self.yb, self.nind2, self.nind_aux2)
        u_boundary = gm_eval(w, self.sig, self.xc, self.yc, self.xb, self.yb)
