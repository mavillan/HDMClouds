...
class HDMClouds():
    ...
    def __init__(self, data, freq=None, n_gaussians=250, alpha=0., lamb=1., 
        ...
        if verbose:
            if self.ndim==2: 
                gp.points_clusters(data, mu_init, db_labels, hdice_keys, wcs=wcs)
            if self.ndim==3:
                pass
        ...
        minsig = np.min(np.abs(sig_global))/10.
...
class HDICE():
    ...
    def __init__(self, 
                 ...
                 bound_points=None, min_num_gaussians=5):
    ...
    def prune(self):
        w = self.w
        mean = np.mean(w**2)
        median = np.median(w**2)
        # all values greater than 1e-3 the mean/median
        mask = w**2 > 1e-3*min(mean,median)
